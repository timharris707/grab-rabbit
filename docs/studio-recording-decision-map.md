...
|---|---|
| Camera devices already feed an `AVCaptureSession` and a floating preview. | `QuickRecorder/AVContext.swift:13-49` configures a video-data output, but its sample callback is entirely reserved/commented out. Studio needs a real frame-consumer path rather than capturing this preview window. |
